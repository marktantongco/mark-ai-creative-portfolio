 ...
 * FAILURE_MODES, and CONTRARIAN_VIEWS.
 *
 * Reframe (this round): the build-time generation pattern is now applied to
 * AUDIT_PERSPECTIVES and FAILURE_MODES (and CONTRARIAN_VIEWS for consistency).
 * They were previously hardcoded in src/lib/subpage-data.ts. Now they are
 * sidecar-driven — adding a 6th animal-metaphor perspective (or a new failure
 * mode) requires ZERO source code changes. Just edit the sidecar JSON and run
 * prebuild.
 *
 * Sidecars:
